...
from ServidorgRPC import ArCondicionado, SistemaControleIncendio, Lampada
from HomeAssistent import SensorTemperatura, SensorFumaca, SensorLuminosidade
...
# # Endpoint para obter dados de temperatura
# @app.route('/api/temperatura', methods=['GET'])
# def obter_temperatura():
#     # Comunicação com o Home Assistant para obter a temperatura real via gRPC
#     with insecure_channel('localhost:50051') as channel:
#         stub = smart_environment_pb2_grpc.HomeAssistantStub(channel)
#         response = stub.ObterDadosAmbiente(smart_environment_pb2.Vazio())
...
# @app.route('/api/controlar-ar-condicionado/<acao>', methods=['POST'])
# def controlar_ar_condicionado(acao):
#     # Comunicação com o Home Assistant para controlar o ar-condicionado via gRPC
#     with insecure_channel('localhost:50051') as channel:
#         stub = smart_environment_pb2_grpc.ArCondicionadoStub(channel)
#         if acao == "ligar":
...
#
#     return jsonify({"status": f"Comando {acao} enviado com sucesso"})
